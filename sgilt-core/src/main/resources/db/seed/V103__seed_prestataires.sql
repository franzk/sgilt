-- V103 : Seed des prestataires de démonstration (15 prestataires).
-- Exécuté uniquement sur les profils `local` et `staging` via spring.flyway.locations.
--
-- Squash de l'ancien couple V101 (insert brut hero_image/photos/youtube_id) +
-- V102 (normalisation des chemins vers R2), remis à plat pour coller au
-- schéma introduit par V13 (colonne `medias` JSONB unifiée). V101/V102 ont
-- été supprimés : ils référençaient des colonnes que V13 a supprimées, et ne
-- pouvaient plus jamais être appliqués sur une base fraîche (V13 s'exécute
-- toujours avant, en version 13 < 101). On ne les a pas édités en place
-- (cassure de checksum Flyway sur les environnements où ils ont déjà tourné) ;
-- `spring.flyway.ignore-migration-patterns=*:missing` permet à Flyway
-- d'ignorer leur absence sur ces environnements. La normalisation des chemins
-- que faisait V102 (`/images/xxx/foo.jpg` → `uploads/foo.jpg`) est appliquée
-- directement dans les `ref` des médias ci-dessous, plus besoin d'un second
-- passage. Toutes les insertions utilisent `ON CONFLICT ... DO NOTHING` pour
-- rester sans effet si ces lignes existent déjà.
--
-- UUIDs de référence :
...
INSERT INTO prestataires (id, utilisateur_id, name, slug, baseline, short_description, category_key, medias, badges, offerings, identity, budget, testimonials, logistics, technical, faq, created_at)
VALUES (
